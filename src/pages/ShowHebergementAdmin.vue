    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb" style="background-color: #a6bfe6 !important">
        <li class="breadcrumb-item">
          <router-link to="/hebergements">Les hébergements</router-link>
        </li>
        <li
        ...
        <div class="col-md-8">
          <b-card no-body>
            <h2 style="margin-left: 20px; margin-top: 5px">
              ...
              <i
                v-if="proprietaire.proprietaire == 0"
                onmouseover="this.style.cursor='pointer'"
                style="color: red; float: right"
                class="fas fa-user-check mr-3 fa-xs"
                @click.once="
                  validateProprietaire(
                    hebergement.proprietaire_id,
                    proprietaire
                  )
                "
                title="Valider l'hébergeur"
              ></i>

              <i
                v-if="hebergement.valide == 0"
                onmouseover="this.style.cursor='pointer'"
                class="fas fa-check-square mr-3 fa-xs"
                style="color: red"
                @click.once="validateHebergement(hebergement.id, hebergement)"
                title="Valider l'hébergement"
              ></i>
            </h2>
        ...
        <div class="container" style="padding-bottom: 20px">
          ...
          <div class="row">
            <div style="padding-bottom: 20px">
              <Vr360
                style="width: 40vw; height: 35vh; padding-bottom: 20px"
                class="col-md-4"
                v-for="(image, k) in hebergement.images_hebergement360"
                v-bind:key="k"
                :imgSrc="`https://etnafes.com/hebergement360/${image.url_image}`"
...
export default {
  ...
  methods: {
    validateProprietaire(id, proprietaire) {
      fetch(`${apiDomain}/api/proprietaire-validate/${id}`, {
        method: "put",
        body: JSON.stringify(this.proprietaire),
        headers: {
    ...
    validateHebergement(id, hebergement) {
      fetch(`${apiDomain}/api/hebergement-validate/${id}`, {
        method: "put",
    ...
    fetchHebergement(id) {
      fetch(`${apiDomain}/api/hebergementadmin/${id}`)
        .then((res) => res.json())
        .then((res) => {
          this.hebergement = res.hebergement;
          this.proprietaire = res.hebergement.proprietaire;
