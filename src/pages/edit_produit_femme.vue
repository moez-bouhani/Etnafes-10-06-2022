          ...
          <template slot="header">
            <h5 class="title" style="padding-top: 15px">
              ...
              <i
                ...
                @click="deleteProduit(id_produit)"
              ></i>
            </h5>
          </template>
...
export default {
  ...
  data() {
    return {
      id_produit: this.$route.params.id,
      loading: false,
      preview_list3600: [],
      images3600: [],
      updateimage: "",
  ...
  methods: {
    deleteProduit(id) {
      if (confirm("Are You Sure?")) {
        fetch(`${apiDomain}/api/delete_produit/${this.$route.params.id}`, {
          method: "delete",
        })
          .then((res) => res.json())
          .then((data) => {
    ...
    Remove(file, index) {
      this.preview_list3600.splice(index, 1);
      this.images3600.splice(index, 1);
